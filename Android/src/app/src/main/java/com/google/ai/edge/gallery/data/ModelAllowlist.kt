...
data class AllowedModel(
  ...
  fun toModel(): Model {
    // Construct HF download url. Use Chinese mirror for better access in China.
    var version = commitHash
    var downloadedFileName = modelFile
    val baseUrl = "https://hf-mirror.com" // Use Chinese mirror
    var downloadUrl =
      url ?: "$baseUrl/$modelId/resolve/$commitHash/$modelFile?download=true"
    ...
    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.S) {
      if (socToModelFiles?.isNotEmpty() == true) {
        socToModelFiles.get(SOC)?.let { info ->
          ...
          downloadUrl =
            info.url
              ?: "$baseUrl/$modelId/resolve/${info.commitHash}/${info.modelFile}?download=true"
